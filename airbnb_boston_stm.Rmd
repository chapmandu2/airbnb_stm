...
```{r}
plot(est_effect, covariate='score', method='continuous', topics=1:4)
```

And now topics 5-8

```{r}
plot(est_effect, covariate='score', method='continuous', topics=5:8)
```

### Focus on topics 5 and 8

What are topics 5 and 8?  These seem to have a positive and negative relationship with price respectively (up vs down slope).  Terms like boston and appartment tend to appear in many topics, so in this case looking at FREX, score and lift is useful as we see words exclusive to these topics.

```{r}
labelTopics(topic_model, c(5,8), n=10)
```

Generate a wordcloud of most common terms for each:

```{r}
cloud(topic_model, topic=5, max.words = 10)
cloud(topic_model, topic=8, max.words = 10)
```

View topic probabilities for the words luxury and studio:

```{r}
tidy(topic_model) %>% 
  dplyr::filter(term %in% c('luxury', 'studio'), topic %in% c(5,8))
```
...
slope_ests <- tidyr::crossing(topic=1:best_k, rep=1:20) %>%
  mutate(slope_est = map2_dbl(topic, rep, ~est_effect$parameters[[.x]][[.y]]$est['score'])) %>%
  group_by(topic) %>%
...
topic_top_terms %>%
  ...
  ggplot(aes(term, beta, fill=slope_est)) +
    ...
    scale_fill_gradient2(mid='lightgray') +
